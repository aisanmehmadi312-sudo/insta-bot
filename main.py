...
async def expand_scenario(update, context):
    ...
    try:
        # پرامپت طلایی و نهایی با بستن تمام راه‌های فرار هوش مصنوعی
...
۲. لحن صریح، قاطع و زنده: لحن تو باید مثل یک متخصصِ حرفه‌ای اما رک و بی‌تعارف باشد. 
   ...
   - ممنوعه: استفاده از افعال ادبی، قدیمی و شاعرانه (مثل "دریابید"، "بیندیشید") مطلقاً ممنوع است.
   - مجاز: کلمات باید قدرتمند، روزمره و با اعتماد به نفس باشند.
۳. پایان‌بندی هوشمند (قانون آهنین): تحت هیچ شرایطی حق نداری کلمات "کامنت"، "سیو"، "لایک"، "اشتراک" یا "بگو" را استفاده کنی. ویدیو را فقط و فقط با یکی از این دو روش تمام کن: یا ارجاع مستقیم به کپشن (مثال: "راز اصلی رو تو کپشن برات شکافتم") یا یک سوال لفاظانه و کنایه‌آمیز که نیازی به جواب مستقیم ندارد.
۴. عبارات زرد مثل "سلام"، "آیا می‌دانستید"، "نتیجه‌گیری" کاملاً ممنوع است.

ساختار سناریو (سریع و ضربه‌ای):
- متن روی تصویر (Hook Visual): یک جمله کوتاه و جنجالی.
- نریشن قلاب (Hook Audio): همان قلاب با لحن قاطع.
...
if __name__ == '__main__':
    ...
    print("🚀 Bot is running with the ultimate strict copywriter prompt!")
    app.run_polling()
