...
async def expand_scenario(update, context):
    ...
    try:
        # پرامپت جدید با قوانین لحن و زمان
...
۱. زمان طلایی (مهم‌ترین قانون): کل متن نریشن باید بین ۴۰ تا حداکثر ۶۰ کلمه باشد (حدود ۲۰ ثانیه). روده درازی نکن، کلمات اضافه را دور بریز و سریع برو سر اصل مطلب.
۲. لحن صریح، قاطع و روان: لحن تو باید مثل یک متخصصِ حرفه‌ای اما رک و بی‌تعارف باشد که مستقیم به لنز دوربین نگاه می‌کند. 
   - ممنوعه: از صمیمیتِ فیک، کرینج و کلمات کوچه بازاری (مثل "ببین رفیق"، "داداش"، "عزیزم") به شدت فرار کن! 
   - ممنوعه: حق نداری کتابی، اتوکشیده و مثل گوینده‌های اخبار حرف بزنی. 
   - مجاز: جملات باید محاوره‌ایِ استاندارد، بسیار کوتاه، کوبنده و با اعتماد به نفسِ بالا باشند.
۳. گداییِ تعامل ممنوع: از عبارات التماس‌گونه مثل "سیو کن"، "بفرست برای دوستات" یا "کامنت بذار" استفاده نکن. در عوض بحث را به صورت نامرئی به کپشن ارجاع بده یا یک سوال تهاجمی بپرس که کاربر از شدت درگیری ذهنی خودش کامنت بگذارد.
۴. عبارات زرد و کلاسیک مثل "سلام"، "آیا می‌دانستید"، "فصل اول"، "نتیجه‌گیری" کاملاً ممنوع است.

ساختار سناریو (سریع و ضربه‌ای):
- متن روی تصویر (Hook Visual): یک جمله کوتاه و جنجالی برای روی ویدیو.
- نریشن قلاب (Hook Audio): همان قلاب جنجالی با لحن قاطع.
- نریشن بدنه (Body): فقط یک دلیل کوبنده و کوتاه (حداکثر ۲ تا ۳ جمله) برای اثبات ادعای کاربر.
- نریشن پایان (Invisible CTA): یک ضربه نهایی و هوشمندانه.

خروجی را به صورت یک متن یکپارچه بنویس. بخش‌های 'متن روی تصویر' و 'نریشن گوینده' را مشخص کن. از ستاره (*) برای بولد کردن استفاده نکن."""

        script = client.chat.completions.create(model="gpt-4o", messages=[{"role": "user", "content": prompt}]).choices[0].message.content.replace('*', '')
        
        # محاسبه دقیق‌تر زمان برای متن‌های کوتاه
...
if __name__ == '__main__':
    ...
    print("🚀 Bot is running with the fast, punchy, no-cringe expert copywriter!")
    app.run_polling()
